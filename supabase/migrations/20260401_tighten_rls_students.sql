...
CREATE POLICY "Students can read own row or admin" ON public.students
  ...
  USING (
    auth.uid() = id
    OR EXISTS(SELECT 1 FROM public.admins WHERE public.admins.id = auth.uid())
  );
...
DROP POLICY IF EXISTS "Allow public read student_answers" ON public.student_answers;
DROP POLICY IF EXISTS "Allow public insert student_answers" ON public.student_answers;
DROP POLICY IF EXISTS "Allow public update student_answers" ON public.student_answers;
DROP POLICY IF EXISTS "Allow public delete student_answers" ON public.student_answers;
