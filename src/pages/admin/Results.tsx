...
export default function Results() {
  ...
  async function fetchCompetitions(silent = false) {
    try {
      const [{ data: comps, error }, { data: summaryRows, error: summaryError }] = await Promise.all([
        supabase
        .from('competitions')
        .select('*')
  ...
  async function loadLeaderboard(compId: string, comp?: Competition, forceRefresh = false, silent = false) {
    ...
    try {
      ...
      const [{ data: submissions, error: subError }, { data: questions, error: qError }] = await Promise.all([
        (supabase as any)
          .from('competition_result_reports')
          .select('student_id, student_name, total_marks, correct_marks, negative_marks, submitted_at, started_at, percentage, rank, is_topper')
            ...
            <TableBody>
              {entries.map((entry, index) => (
                <TableRow key={entry.student_id} className="hover:bg-primary/5">
                  ...
                  <TableCell className="font-bold whitespace-nowrap">{entry.student_name}</TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="flex items-center gap-2">
                      <div className="w-16 lg:w-24 h-2 bg-muted/50 rounded-full overflow-hidden">
                        <div
